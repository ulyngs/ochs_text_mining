...
by_line <- vroom("data_processed/texts_line_by_line.csv")
...
### Welcome to the Muktabodha Dashboard!

This is an example of using a flexdashboard to interact with the muktabodha data
...
```{r}
term_frequencies %>% 
  select(document, total) %>% 
  distinct() %>% 
  ggplot() +
    geom_col(aes(x = document, y = total, fill = document)) +
...
renderDataTable({
    ...
    data <- term_frequencies %>%
      ...
      mutate(term_frequency = round(term_frequency, 5))
